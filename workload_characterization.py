...
class LSTM_pytorch_python3_10(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers):
        super(LSTM_pytorch_python3_10, self).__init__()
...
class WorkloadCharacterization:
    ...
    @staticmethod
    def dnn_clf(x_axis, y_axis, clfpath=None, hidden_layers=(100,), activation='relu', solver='sgd', search=False):
        """
        DNN_clf: Deep Neural Network classifier
        """
        x_train, x_test, y_train, y_test = tts(x_axis, y_axis, test_size=0.3)
        model = MLPClassifier(hidden_layer_sizes=hidden_layers, activation=activation, solver=solver, shuffle=True)
        if search:
            tuned_parameters=[
                {'hidden_layer_sizes': [(50,), (100,), (200,)],
                 ...
                 'shuffle':[True, False]},
                {'hidden_layer_sizes': [(25,25), (50,25), (50,50)],
                 'activation': ['relu', 'logistic', 'tanh'],
                 'solver': ['adam', 'sgd','lbfgs'],
                 'alpha': [0.0001, 0.001, 0.01], 
                 'batch_size':['auto', 8,16,32,64],
                 'shuffle':[True, False]},
                {'hidden_layer_sizes': [(10,20,10), (15,25,10), (20,20,15),(20,25,25)],
                 'activation': ['relu', 'logistic', 'tanh'],
                 'solver': ['adam', 'sgd','lbfgs'],
                 'alpha': [0.0001, 0.001, 0.01], 
                 'batch_size':['auto', 8,16,32,64],
            ...
            model = GridSearchCV(estimator=model, param_grid=tuned_parameters, cv=5, n_jobs=-1, pre_dispatch='0.5*n_jobs')
        model.fit(x_train, y_train)
        y_pred = model.predict(x_test)
        print(f"the accuracy of DNN classifier is {accuracy_score(y_test, y_pred)}")
        if hasattr(model, 'best_params_'):
            print(f"the DNN grid search best params is: {model.best_params_}")  
        if clfpath is not None:
            joblib.dump(model, clfpath)
        return model
    
    @staticmethod
    def lstm_clf(x_axis, y_axis, clfpath=None, lstm_units=20, epochs=15, batch_size=32):
        """
        lstm_clf: Long Short-Term Memory (LSTM) classifier (small)
        """
        ...
        x_train, x_test, y_train, y_test = tts(x_lstm, y_axis, test_size=0.3)
        model = Sequential()
        model.add(LSTM(units=lstm_units, input_shape=(1, x_scaled.shape[1])))
        model.add(Dense(units=1, activation='sigmoid'))
        model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
        model.fit(x_train, y_train, epochs=epochs, batch_size=batch_size, verbose=1)
        loss, accuracy = model.evaluate(x_test, y_test)
        print(f"the accuracy of LSTM classifier is {accuracy}")
        if clfpath is not None:
            model.save(clfpath)
        return model

    @staticmethod
    def lstm_large_clf(x_axis, y_axis, clfpath=None, lstm_units=50, epochs=15, batch_size=32):
        """
        lstm_large_clf: Long Short-Term Memory (LSTM) classifier (bigger)
        """
        from tensorflow.keras.models import Sequential
        from tensorflow.keras.layers import LSTM, Dense
        from sklearn.preprocessing import MinMaxScaler
        scaler = MinMaxScaler()
        x_scaled = scaler.fit_transform(x_axis)
        x_lstm = x_scaled.reshape(x_scaled.shape[0], 20, x_scaled.shape[1])
        x_train, x_test, y_train, y_test = tts(x_lstm, y_axis, test_size=0.3)
        model = Sequential()
        model.add(LSTM(units=lstm_units, input_shape=(20, x_scaled.shape[1]), return_sequences=True))
        ...
        model.fit(x_train, y_train, epochs=epochs, batch_size=batch_size, verbose=1)
        loss, accuracy = model.evaluate(x_test, y_test)
        print(f"the accuracy of LSTM_larger classifier is {accuracy}")
        if clfpath is not None:
            model.save(clfpath)
        return model

    @staticmethod
    def lstm_pytorch(x_axis, y_axis, clfpath=None, hidden_size=64, num_layers=1):
        window_size = 10
        for i in range(len(x_axis) - window_size):
            x.append(x_axis[i:i+window_size])
            y.append(y_axis[i+window_size])
        x = torch.tensor(x, dtype=torch.float32)
        y = torch.tensor(y, dtype=torch.float32).view(-1, 1)
        x_train, x_test, y_train, y_test = tts(x, y, test_size=0.3)
        model = LSTM_pytorch_python3_10(window_size, hidden_size, num_layers)
        loss_fn = nn.BCELoss()
    ...
    def train(self, data_path, feature_selection=False, search=False):
        ...
        for workload, group in self.dataset.groupby('workload.type'):
            ...
            print('x_app shape is:',x_app.shape)
            print('y_app shape is:',y_app.shape)
            print(x_app[0])
            print('*********************************')
            print(y_app[0])
            ...
            if workload == "default":
                ...
                self.dnn_clf(x_app,y_app,clf_path,search=False)
                #self.lstm_clf(x_app, y_app, clf_path)
                #self.lstm_large_clf(x_app,y_app,clf_path)
                #self.lstm_pytorch(x_app,y_app,clf_path)
